...
This codec performs no feature transform: it simply flattens each input chunk (and optionally casts dtype). It is
useful for:
- bypassing compression/encoding for specific signals,
- debugging the end-to-end token/embedding pipeline,
- keeping signals in (almost) native space while still using the same embedding/caching interfaces as other codecs.
"""
...
class IdentityCodec:
    ...
    This codec does not perform any transform in feature space: it simply flattens the input array and optionally
    changes dtype.

    It is useful when you want to:
      - bypass DCT3D or any compression,
      - or handle certain signals in native space, while still going through the same embedding / caching machinery.

    Parameters
    ----------
    out_dtype : np.dtype, optional
        Dtype of the encoded vector. Calculations are trivial here, so `out_dtype` is both the internal and output
