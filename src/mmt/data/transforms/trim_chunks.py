# src/mmt/data/transforms/trim_chunks.py
...
class TrimChunksTransform:
    ...
    max_chunks : int
        Maximum total number of chunks (input + actuator) to keep.
    """
    ...
    def _compute_pos(self, t_out_end: float, t_start: float) -> int:
        """
        Compute pos ID (same formula as in BuildTokensTransform).
        """
        if t_start is None:
            raise ValueError(
    ...
    def _trim_role(
        ...
        Filtering is based on pos >= -max_chunks.
        """
        if not chunks:
            return chunks
    ...
    def __call__(self, window: Dict[str, Any]) -> Dict[str, Any]:
        ...
        trimmed_input = self._trim_role(input_chunks, t_cut, t_out_end)
        trimmed_act = self._trim_role(actuator_chunks, t_cut, t_out_end)
