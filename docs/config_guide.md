...
6. `tasks_overrides/<task>/embeddings_overrides.yaml` *(optional; typically used for pretrain/finetune/eval, not used during tuning)*
...
Task-specific tuned overrides belong in `tasks_overrides/<task>/embeddings_overrides.yaml`.
...
- `tasks_overrides/<task>/embeddings_overrides.yaml` contains **only per-signal overrides**.
...
`tune_dct3d` writes `embeddings_overrides.yaml` to `tasks_overrides/<task>/`.
...
```
This writes `tasks_overrides/<task>/embeddings_overrides.yaml`.
