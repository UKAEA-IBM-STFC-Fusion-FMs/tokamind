...
- Type: `str` — one of `deterministic`, `gaussian`
- Default: `deterministic`
- Description: output head type. `deterministic` emits a single prediction per output (`pred[sid]`). `gaussian`
  emits a per-coordinate diagonal Gaussian (mean + scale); `pred[sid]` stays the mean, and the distribution
  parameters are exposed as `pred_dist` for probabilistic losses (`embed_gaussian_crps`,
  `native_sparse_sample_crps`) and eval CRPS. The mean head shares the deterministic adapter's layout, so a
  deterministic checkpoint warm-starts the Gaussian mean head.

### `model.output_adapters.gaussian.raw_scale_bias_init`
- Type: `float`
- Default: `-5.0` (constructor default; the shipped configs use `-1.0`)
- Description: initial bias of the raw-scale head, controlling the initial predictive sigma
  (`sigma ≈ softplus(raw_scale_bias_init) + eps`). Lower → start near-deterministic (suited to warm-starting a
  Gaussian head from an MSE-trained run); higher → wider initial spread (suited to from-scratch CRPS training).
  Only read when `type: gaussian`.
...
| `embed_gaussian_crps` | Closed-form Gaussian CRPS in embedding space. **Requires `model.output_adapters.type: gaussian`** (reads `pred_dist`). No sampling, no decoding — the preferred embedding-space probabilistic objective. |
| `native_sparse_sample_crps` | Sample-based CRPS in native standardized space. **Requires a Gaussian head and decoders.** Draws `n_samples` reparameterized samples, decodes each, and scores marginal CRPS with the same NaN masking as `native_sparse_mse`. The primary native-space probabilistic objective; works for DCT3D, VAE, and identity outputs. |
...
Probabilistic terms (`embed_gaussian_crps`, `native_sparse_sample_crps`) require `model.output_adapters.type: gaussian`; the config validator rejects them against a `deterministic` head with an error naming both the term and the knob to flip.
...
### `train.loss.terms[].n_samples`
- Type: `int` (`>= 2`)
- Description: number of reparameterized samples for sample-based probabilistic terms (`native_sparse_sample_crps`).
  Required for those terms and rejected on all others (the validator fails loudly on a misplaced key). Default
  `16` when omitted. Closed-form terms (`embed_gaussian_crps`) take no `n_samples`.
...
### `eval.compute_metrics.crps.enable`
- Type: `bool`
- Default: `false`
- Description: master on/off for the MMT-native CRPS diagnostic. When `true`, CRPS/NCRPS are written at the
  granularity selected by the same `per_task`/`per_shot`/`per_window`/`per_timestamp` flags, as parallel
  `crps_*_metrics.csv` files (`crps_task_metrics.csv`, `crps_shot_metrics.csv`, `crps_window_metrics.csv`,
  `crps_timestamps_metrics.csv`), plus a shot-weighted task-level `CRPS_mean`/`NCRPS_mean` logged as
  `Additional task metrics`. CRPS/NCRPS aggregate by plain mean over the same window→shot→signal→task hierarchy as
  NRMSE (not RMSE's square/sqrt). For a Gaussian head, CRPS is estimated from `n_samples` decoded samples; for a
  deterministic head it reduces to MAE (`|pred − target|`, no sampling). NCRPS divides by the same per-signal std
  used for NRMSE, so the two share a scale. Does not affect the tokamark point metrics.

### `eval.compute_metrics.crps.n_samples`
- Type: `int` (`>= 2`)
- Default: `50` (in the shipped eval configs)
- Description: number of decoded predictive samples for the CRPS estimate. Required (`>= 2`) for a Gaussian head;
  ignored for a deterministic head.
