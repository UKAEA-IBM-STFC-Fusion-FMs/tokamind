...
def main() -> None:
    ...
    do_eval = bool(
        ...
        or bool((cfg_compute_metrics.get("crps") or {}).get("enable", False))
    ...
    if do_eval:
        ...
        if "crps_metrics" in result:
            logger.info(
                "Additional task metrics: %s",
                {
                    "CRPS_mean": result["crps_metrics"]["CRPS_mean"],
                    "NCRPS_mean": result["crps_metrics"]["NCRPS_mean"],
                },
            )
        logger.info("Task metrics dir: %s", result.get("metrics_task_dir"))
    else:
        logger.info(
            "[eval] compute_metrics: per_task, per_shot, per_window, per_timestamp, CRPS, GS residual, smoothness, "
